...
const latLonToQTH = (lat: number, lon: number, precision: number = 6) => {
  if (precision % 2 !== 0 || precision < 2) {
    throw new Error('Precision must be an even number >= 2')
  }
  ...
  // Field (A-R)
  locator += String.fromCharCode('A'.charCodeAt(0) + Math.floor(adjLon / 20))
  locator += String.fromCharCode('A'.charCodeAt(0) + Math.floor(adjLat / 10))
  ...
  adjLon %= 20
  adjLat %= 10
  locator += Math.floor(adjLon / 2).toString()
  locator += Math.floor(adjLat / 1).toString()
  ...
  adjLon %= 2
  adjLat %= 1
  locator += String.fromCharCode('a'.charCodeAt(0) + Math.floor(adjLon / (2 / 24)))
  locator += String.fromCharCode('a'.charCodeAt(0) + Math.floor(adjLat / (1 / 24)))
  ...
  adjLon %= 2 / 24
  adjLat %= 1 / 24
  locator += Math.floor(adjLon / (2 / 24 / 10)).toString()
  locator += Math.floor(adjLat / (1 / 24 / 10)).toString()
...
const qthToLatLon = (
  ...
  if (loc.length >= 6) {
    // Subsquare (a-x)
    lon += (loc[4].toLowerCase().charCodeAt(0) - 97) * (2 / 24)
    lat += (loc[5].toLowerCase().charCodeAt(0) - 97) * (1 / 24)
  }

  if (loc.length >= 8) {
    // Extended square (0-9)
    lon += parseInt(loc[6], 10) * (2 / 24 / 10)
    lat += parseInt(loc[7], 10) * (1 / 24 / 10)
  }
